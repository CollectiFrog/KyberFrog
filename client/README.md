...
(PCSceneJar, PCSceneCour, …), serves a small **web UI**, and supervises **N
`kyclient` viewers** — each connected to one transmitter published by a Server on
the regie.
...
are persisted, so a scene PC comes back on its own after a reboot or a regie
...
Browse `http://<scene-pc>:<web_port>/` (default port **7701**, bound on the LAN).
...
Every change is written to `scene-agent.toml`, so it survives a reboot. Picking
...
`%APPDATA%\kyberfrog\scene-agent.toml`, created with defaults on first run. You
...
only visible if a viewer is dropped to a window — fine for a scene PC.
