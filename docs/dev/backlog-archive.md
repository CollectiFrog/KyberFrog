...
| #28-2 | Zero-copy D3D11 on reception — libVLC renders straight into the shared Spout texture, GPU→GPU, the default on Windows. Architecture: [plan](plan-spout-zerocopy.md) | 0.6.0 |
...
| #32 | V4L2 cameras on Linux — picker by card name (`ffmpeg -sources v4l2`), `EnumerateDisplays` honouring the pin, pin by node path (`/dev/…`, symlinks followed) and `camera_options` for the demuxer. Validated end to end on 2026-09-26 on a Debian 13 VM (v4l2loopback and a passed-through webcam, test pattern received on Windows) and on the Pi 5 + C790 (the HDMI source reaches a Windows viewer; throughput is #49). *Without a graphical session the user needs `video`: #50* | unreleased |
...
| — | **Linux amd64** — a standalone `.deb` built and released next to the Windows installer, validated end to end on a Debian 13 / Xfce VM. Architecture: [plan](plan-linux-amd64.md) · [status](todo-linux.md) | 0.6.0 |
| #28-1 | GPU encoder by default — machine setting *Options → Video encoding*, `auto` = AMF / NVENC from the vendor of DXGI adapter 0, else x264, with an automatic x264 fallback. ~4 ms Spout → Spout instead of ~26 ms. Checked on 2026-09-26: three transmitters at once (Spout and screen on AMF, a webcam on the x264 fallback), no blocking, blur or banding at 20 Mbps, a few dropped frames. *Capture sources stay on x264: #48-A* | 0.6.0 |
| #35 | arm64 — S1 of #46: aarch64 fork bundle, `kyberfrog_<version>_arm64.deb` for Pi OS Trixie, installed and started on a Pi 5 on 2026-09-23. *The x264-only performance is #46 S0* | unreleased |
| #29 | Self-hosted web fonts — Inter and Londrina Solid ship inside `ui/dist` (`@fontsource`), no call to Google Fonts. Checked on 2026-09-26 with the network cut: the dashboard keeps its typefaces | unreleased |
