...
| #32 | V4L2 cameras on Linux — the picker lists the devices (`ffmpeg -sources v4l2`), a pinned camera is the only source a client sees (`kymedia` `1cd85a8`). Checked on 2026-09-26 on a Debian 13 VM: `KF Test Cam` (v4l2loopback) and a passed-through webcam listed, test pattern received on Windows. *Needs the user in `video` without a graphical session: #49* | unreleased |
