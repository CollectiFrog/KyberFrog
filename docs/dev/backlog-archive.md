...
| #28-1 | GPU encoder by default — machine setting *Options → Video encoding*, `auto` = AMF / NVENC from the vendor of DXGI adapter 0, else x264, with an automatic x264 fallback. ~4 ms Spout → Spout instead of ~26 ms. *Leftover checks (quality, several transmitters, screen source) are in the validation queue* | 0.6.0 |
