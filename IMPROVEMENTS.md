...
#### 17. Rework remote desktop (remote-control viewer) — phases 2–3 ouvertes
- **What:** le viewer remote-control, utilisable en paysage → paysage depuis la
  0.4.0. Restent ouverts : les écrans **verticaux** (rotation non appliquée),
  `Ctrl+Alt+F` sous keyboard grab, et l'accélération pointeur Windows.
- **Why important:** remote desktop over QUIC sans outil tiers — feature
  attendue et différenciante.
- **Détail :** audit du 2026-07-02 (chemin souris tracé de bout en bout, causes
  racines B1–B5) et plan en 3 phases dans
  [docs/dev/plan-remote-desktop.md](docs/dev/plan-remote-desktop.md).

### Latence (chaîne fork)

#### 28. Latence bout-en-bout — analyse et leviers
- **What:** l'analyse de la chaîne complète (capture → encode → QUIC → décode →
  Spout → hôte) et les leviers pour la réduire. **La latence est la priorité
  n°1 du projet** : les arbitrages d'archi s'y réfèrent au lieu de re-dériver la
  chaîne à chaque fois.
- **Why deferred:** travail **côté fork** (kymedia / kyctl), pris en charge par
  l'opérateur. Ne bloque pas #27.
- **Détail :** les 5 leviers, leur gain estimé et le levier 2 déjà livré
  (zero-copy D3D11, 2026-07-18) dans
  [docs/dev/plan-latency.md](docs/dev/plan-latency.md).
...
#### 27. Passthrough Spout (2 modes : émission / réception)
- **What:** rendre Kyber utilisable depuis Resolume / TouchDesigner / OBS comme
  on utiliserait NDI — **deux interrupteurs indépendants**, un par sens, sans
  ajouter une seule copie au chemin natif.
- **Why:** remplace la demande initiale « plugin Resolume (FFGL) + plugin
  TouchDesigner » — l'exploration du 2026-07-17 a conclu qu'**aucun des deux ne
  doit être écrit**.
- **Statut :** archi arrêtée le 2026-07-17, implémentation non commencée.
- **Détail :** décision « deux interrupteurs » et ses 5 raisons, filtre
  anti-boucle à 2 couches, 3 topologies, plan de test E2E, doc utilisateur —
  [docs/dev/plan-spout-passthrough.md](docs/dev/plan-spout-passthrough.md).
...
- **#8** Spout output depuis un viewer — feature fork (crate `kyspout`, smem dans
  ...
  **Limitation connue acceptée (non corrigée) :** le transmetteur se fige et
  ...
  change en cours de stream. **Le round-trip CPU est supprimé depuis le
  2026-07-18** (zero-copy D3D11, output callbacks libVLC 4) : voir #28 levier 2
  et [`docs/dev/plan-spout-zerocopy.md`](docs/dev/plan-spout-zerocopy.md). ✅
